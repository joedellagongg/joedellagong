      ...
      >
        <div className="flex-1 flex items-center justify-center w-full">
          <Image
            ...
            width={width}
            height={50}
            className={`object-contain max-h-[44px] group-hover:scale-110 transition-transform duration-200 ${dark ? "brightness-90" : ""}`}
          />
        </div>
        <span
          className={`${poppins.className} text-[10px] font-medium truncate w-full text-center leading-none ${dark ? "text-gray-500" : "text-gray-400"}`}
        >
          {alt}
        </span>
      </div>
      ...
      <div className="embla" ref={forwardRef}>
        <div className="embla__container">
          {[...techItems, ...techItems].map((item, i) => (
            <TechSlide key={i} {...item} dark={dark} />
          ))}
        </div>
      </div>
  ...
  return (
    <div className={`flex justify-center transition-colors duration-300 ${bg}`}>
      <div className="w-full xl:w-[80%] scroll-smooth">
        ...
        {isOpen && (
          <div
            className={`fixed inset-0 z-40 flex flex-col items-center justify-center gap-8 backdrop-blur-sm transition-colors duration-300
